...
mod tests {
    ...
    fn create_counter_and_increment_and_reset() {
        ...
        reset(&mut app, &manager_contract, "contract1".to_string(), 127);
        let res = get_contracts(&app, &manager_contract);
        let res = get_count(&app, res.contracts[0].1.address.as_str());
        assert_eq!(res.count, 127);
    }
    ...
    fn create_two_counters_and_increment_each() {
        ...
        let res = get_contracts(&app, &manager_contract);
        let res1 = get_count(&app, res.contracts[0].1.address.as_str());
        let res2 = get_count(&app, res.contracts[1].1.address.as_str());
